//======================================================================
// QUYỀN LỰC BÓNG TỐI - CLIENT LOGIC (NÂNG CẤP BOT & BỎ QUA)
//======================================================================
...
const gameElements = { roundIndicator: document.getElementById('current-round'), decreeDisplay: document.getElementById('decree-display'), decreeTitle: document.getElementById('decree-title'), decreeDescription: document.getElementById('decree-description'), playersContainer: document.getElementById('players-container'), phaseTitle: document.getElementById('phase-title'), chaosControls: document.getElementById('chaos-phase-controls'), timerDisplay: document.getElementById('timer-display'), choiceButtons: document.getElementById('player-choice-buttons'), messageArea: document.getElementById('message-area') };
...
socket.on('newRound', data => { state.gamePhase = 'choice'; state.players = data.players; gameElements.roundIndicator.textContent = data.roundNumber; gameElements.phaseTitle.textContent = 'Bước 1: Lựa Chọn Bí Mật'; gameElements.decreeDisplay.style.display = 'none'; gameElements.chaosControls.style.display = 'none'; clearInterval(state.countdownTimer); renderPlayerCards(); gameElements.choiceButtons.innerHTML = `<button class="choice-buttons loyal" onclick="sendPlayerChoice('Cống Hiến')">Cống Hiến</button><button class="choice-buttons corrupt" onclick="sendPlayerChoice('Tham Nhũng')">Tham Nhũng</button><button class="choice-buttons blank" onclick="sendPlayerChoice('Phiếu Trống')">Phiếu Trống</button>`; logMessage('info', `--- Vòng ${data.roundNumber} bắt đầu! Hãy đưa ra lựa chọn của bạn. ---`); });
...
socket.on('decreeRevealed', data => { gameElements.decreeTitle.textContent = data.decree.name; gameElements.decreeDescription.textContent = data.decree.description; gameElements.decreeDisplay.style.display = 'block'; logMessage('warning', `📜 **${data.drawerName}** đã bốc được Sắc Lệnh: **${data.decree.name}**!`); });
socket.on('chaosPhaseStarted', data => {
    state.gamePhase = 'chaos';
    gameElements.phaseTitle.textContent = "Bước 2: Giai Đoạn Hỗn Loạn!";
    gameElements.choiceButtons.innerHTML = '';
    gameElements.chaosControls.style.display = 'block';
    const totalHumans = state.players.filter(p => !p.isBot && !p.disconnected).length;
    let h = `<div id="timer-display">${data.duration}</div><div class="chaos-actions"><button id="challenge-btn" onclick="showTargetSelection('challenge')">Thách Đấu</button><button id="teamup-btn" onclick="showTargetSelection('teamup')">Ghép Đội</button></div><button id="skip-chaos-btn" class="skip-button" onclick="voteToSkipChaos()">Bỏ Qua <span id="skip-vote-count">(0/${totalHumans})</span></button>`;
    gameElements.chaosControls.innerHTML = h;
    let t = data.duration;
...
// [NÂNG CẤP] Lắng nghe sự kiện cập nhật số phiếu bỏ qua
socket.on('updateSkipVoteCount', (count, total) => {
...
socket.on('gameOver', data => { state.gamePhase = 'gameover'; gameElements.phaseTitle.textContent = '🏆 TRÒ CHƠI KẾT THÚC 🏆'; gameElements.choiceButtons.innerHTML = ''; let m = ''; if (data.winner) m = `🎉 **${data.winner.name}** đã chiến thắng! 🎉`; else if (data.loser) m = `☠️ **${data.loser.name}** đã thất bại! ☠️`; logMessage('warning', m); gameElements.choiceButtons.innerHTML = `<h2 class="warning">${m}</h2>`; });
...
function sendPlayerChoice(choice) { gameElements.choiceButtons.innerHTML = '<p class="info">Đã gửi lựa chọn! Đang chờ...</p>'; socket.emit('playerChoice', { roomCode: state.currentRoomCode, choice }); }
function showTargetSelection(actionType) { let t = actionType === 'challenge' ? 'Thách Đấu Ai?' : 'Ghép Đội Với Ai?'; let c = '<div class="player-selection-grid">'; state.players.forEach(p => { if (p.id !== state.myId && !p.disconnected) c += `<button onclick="requestChaosAction('${p.id}', '${actionType}')">${p.name}</button>`; }); c += '</div>'; createModal(t, c); }
function requestChaosAction(targetId, actionType) { if (actionType === 'challenge') { let g = `<p>Bạn thách đấu <strong>${state.players.find(p=>p.id===targetId).name}</strong>. Đoán:</p><button class="choice-buttons loyal" onclick="submitChallengeGuess('${targetId}', 'Cống Hiến')">Cống Hiến</button><button class="choice-buttons corrupt" onclick="submitChallengeGuess('${targetId}', 'Tham Nhũng')">Tham Nhũng</button><button class="choice-buttons blank" onclick="submitChallengeGuess('${targetId}', 'Phiếu Trống')">Phiếu Trống</button>`; createModal("Đưa Ra Phán Đoán", g); } else { socket.emit('requestChaosAction', { roomCode: state.currentRoomCode, targetId, actionType }); closeModal(); } }
function submitChallengeGuess(targetId, guess) { socket.emit('requestChaosAction', { roomCode: state.currentRoomCode, targetId, actionType: 'challenge', guess }); closeModal(); }
function voteToSkipChaos() { socket.emit('playerVotedToSkip', state.currentRoomCode); const b = document.getElementById('skip-chaos-btn'); if (b) { b.disabled = true; b.textContent = 'Đã bỏ phiếu...'; } }
function selectAmnesiaTarget(playerId) { if (!window.amnesiaSelection) window.amnesiaSelection = []; const i = window.amnesiaSelection.indexOf(playerId); if (i > -1) { window.amnesiaSelection.splice(i, 1); document.getElementById(`amnesia-target-${playerId}`).classList.remove('selected'); } else if (window.amnesiaSelection.length < 2) { window.amnesiaSelection.push(playerId); document.getElementById(`amnesia-target-${playerId}`).classList.add('selected'); } document.getElementById('amnesia-status').textContent = `Đã chọn: ${window.amnesiaSelection.map(id => state.players.find(p=>p.id===id).name).join(', ') || '(Chưa ai)'}`; const b = document.getElementById('amnesia-confirm-btn'); b.disabled = window.amnesiaSelection.length !== 2; b.onclick = () => { socket.emit('amnesiaAction', { roomCode: state.currentRoomCode, player1Id: window.amnesiaSelection[0], player2Id: window.amnesiaSelection[1] }); closeModal(); delete window.amnesiaSelection; }; }